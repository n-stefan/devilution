                 ...
                 shine_global_config *config )
{
  ...
  */
  ...
  if ( temp )
    config->l3loop.en_tot[gr] = (int) (log((double)temp * 4.768371584e-7) / LN2); /* 1024 / 0x7fffffff */
  else
  ...
  for(sfb=21; sfb--; )
  {
    ...
    if ( temp )
      config->l3loop.en[gr][sfb] = (int) (log((double)temp * 4.768371584e-7) / LN2); /* 1024 / 0x7fffffff */
    else
      config->l3loop.en[gr][sfb] = 0;

    if ( l3_xmin->l[gr][ch][sfb])
      config->l3loop.xm[gr][sfb] = (int) (log( l3_xmin->l[gr][ch][sfb] ) / LN2);
    else
...
int new_choose_table( int ix[GRANULE_SIZE], unsigned int begin, unsigned int end )
{
  ...
  if(max<15)
  {
    ...
    for ( i =14; i--; )
      if ( (int) shine_huffman_table[i].xlen > max )
      {
        choice[0] = i;
        break;
      }
  ...
  else
  {
    ...
    for(i=15;i<24;i++)
      if((int) shine_huffman_table[i].linmax>=max)
      {
        choice[0] = i;
        break;
      }
