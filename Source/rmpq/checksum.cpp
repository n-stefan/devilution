...
void MD5::process(void const* _buf, uint32_t size) {
  uint8_t const* buf = (uint8_t*)_buf;
  while (size) {
    uint32_t cur = 64 - bufSize;
...
std::string MD5::format(void const* digest)
{
  ...
  uint8_t const* d = (uint8_t*)digest;
...
uint64_t jenkins(void const* buf, uint32_t length) {
  ...
  case  0: return (uint64_t(b) << 32) | uint64_t(c);
  }
