...
async function run_build(flags, oname, dirs) {
  ...
  async function handle_file(name) {
    ...
    if (statSrc.isDirectory()) {
      ...
      if (rebuild || !statDst || srcTime > statDst.mtime) {
        ...
        const cmd = `emcc -c ${name} -o ${out} ${name.match(/\.(?:cpp|cc)$/i) ? "--std=c++17 " : ""}-DNO_SYSTEM -DEMSCRIPTEN -Wno-logical-op-parentheses ${flags} -I.`; //-fPIC
        try {
  ...
  const cmd = `emcc ${link_list.join(" ")} -o ${oname}.js -s EXPORT_NAME="${oname}" ${flags} -s WASM=1 -s MODULARIZE=1 -s NO_FILESYSTEM=1 --post-js ./module-post.js -s ALLOW_MEMORY_GROWTH=1 -s TOTAL_MEMORY=134217728 -s DISABLE_EXCEPTION_CATCHING=0`; //-s BINARYEN_TRAP_MODE='clamp' -s RELOCATABLE=1 -s EXPORTED_FUNCTIONS=_DApi_Char
...
run_build('-O3 -DZ_SOLO', 'Diablo', ['Source']).catch(e => console.error(e.message));
//run_build('-O3 -DZ_SOLO -DSPAWN', 'DiabloSpawn', ['Source']).catch(e => console.error(e.message));
//run_build('-O3 -DZ_SOLO', 'MpqCmp', ['Source/rmpq', 'Source/zlib', 'mpqcmp']).catch(e => console.error(e.message));
